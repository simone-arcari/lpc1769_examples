...
**	Compiler: GCC
**
**	Date/Time: 08/gen/07 17:12:33
**
**	Abstract:
**
**		HD44780.c source file.
**
**	Settings:
**
**	Contents:
...
void lcd_init()
{
    ...
    lcd_cmd_hf(0x30);       //-----     "            "              "               "
    lcd_cmd_hf(0x20);       //-----     "            "              "               "
    lcd_cmd(0x28);          //-----Selecting 16 x 2 LCD in 4Bit mode with 5x7 pixels
    lcd_cmd(0x0C);          //-----Display ON Cursor OFF Blinking OFF
    lcd_cmd(0x01);          //-----Clear display
    lcd_cmd(0x06);          //-----Cursor Auto Increment
    lcd_cmd(0x80);          //-----1st line 1st location of LCD 0 offset 80h
}
...
void GoToLine(unsigned char line, unsigned char col){
	...
	lcd_cmd(DDRAM_ADDRESS(address));		// Disabled for 8 char LCD DM0810
}
