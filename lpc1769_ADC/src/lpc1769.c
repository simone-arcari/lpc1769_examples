...
{
	char digit;
	unsigned long valDisplay=0;

	if(FIO2PIN & 0x0400)		// test sul pin P2.10 (cifra 1, più significativa)
	{
		//valDisplay = ADCvalue*99/4095;							// calcolo per riportare i valori [0:4095] a [0:99] (trimmer)
		valDisplay = (((float)ADCvalue*3300.0/4095.0)-400.0)/19.5;	// calcolo per riportare i valori di temperatura a [0:99] (sensore)
